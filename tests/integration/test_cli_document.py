...
def test_document_pass_updates_docs_and_stays_in_final_review(tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        ...
        [
            ...
            (
                "from pathlib import Path; "
                "Path('docs/changelog.md').write_text('updated docs\\n', encoding='utf-8'); "
                "print('documented')"
            ),
            "--json",
        ],
    ...
    assert '"role": "documenter"' in result.stdout
    assert '"docs/changelog.md"' in result.stdout
...
def test_document_pass_blocks_when_editing_non_doc_paths(tmp_path: Path) -> None:
    repository_root = tmp_path / "repo"
    _create_task(repository_root)
    _write_state(repository_root, "final_review")
    ...
    result = runner.invoke(
        ...
        [
            ...
            (
                "from pathlib import Path; "
                "Path('src/leak.py').parent.mkdir(parents=True, exist_ok=True); "
                "Path('src/leak.py').write_text('bad\\n', encoding='utf-8'); "
                "print('outside docs scope')"
            ),
            "--json",
        ],
    )
    ...
    assert '"src/leak.py is outside the editable scope for documenter."' in result.stdout
